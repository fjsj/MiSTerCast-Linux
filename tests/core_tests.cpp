...
int main(){
 ...
 int server=socket(AF_INET,SOCK_DGRAM,0);sockaddr_in address{};address.sin_family=AF_INET;address.sin_addr.s_addr=htonl(INADDR_LOOPBACK);if(server>=0&&bind(server,reinterpret_cast<sockaddr*>(&address),sizeof(address))==0){socklen_t alen=sizeof(address);getsockname(server,reinterpret_cast<sockaddr*>(&address),&alen);timeval timeout{2,0};setsockopt(server,SOL_SOCKET,SO_RCVTIMEO,&timeout,sizeof(timeout));bool sawMode=false,sawFrame=false,sawAudio=false,sawClose=false;std::thread endpoint([&]{uint8_t packet[2048];sockaddr_storage peer{};socklen_t plen=sizeof(peer);for(int i=0;i<12&&!sawClose;++i){auto n=recvfrom(server,packet,sizeof(packet),0,reinterpret_cast<sockaddr*>(&peer),&plen);if(n<=0)break;if(packet[0]==2){uint8_t ack[13]{};sendto(server,ack,sizeof(ack),0,reinterpret_cast<sockaddr*>(&peer),plen);}else if(packet[0]==3)sawMode=true;else if(packet[0]==7)sawFrame=true;else if(packet[0]==4)sawAudio=true;else if(packet[0]==1)sawClose=true;}});{GroovyTransport transport;CHECK(transport.open("localhost",48000,error,ntohs(address.sin_port)));Modeline tiny{"tiny",1,2,3,4,5,2,3,4,5,false};CHECK(transport.switchMode(tiny,error));std::vector<uint8_t> pixels(12,42);CHECK(transport.sendFrame(1,0,pixels,error));int16_t sound[4]{};CHECK(transport.sendAudio(sound,4,error));transport.close();}endpoint.join();CHECK(sawMode&&sawFrame&&sawAudio&&sawClose);}if(server>=0)close(server);
