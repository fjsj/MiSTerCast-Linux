...
# IPC namespace (MIT-SHM capture), the X11 socket, and the PulseAudio or
# pipewire-pulse socket. It runs as the invoking user, and settings persist
# in the same mistercast config directory as a native install.
#
...
if [ -z "${DISPLAY:-}" ]; then
  echo 'DISPLAY is not set. Log into an X11/Xorg session and run this from it.' >&2
  exit 1
fi
