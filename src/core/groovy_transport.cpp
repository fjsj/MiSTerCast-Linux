...
void GroovyTransport::drainStatus()noexcept{if(fd_<0)return;uint8_t ack[32];for(;;){auto received=recv(fd_,ack,sizeof(ack),MSG_DONTWAIT);if(received<0){if(errno==EAGAIN||errno==EWOULDBLOCK)return;return;}if(received==13)misterAudioEnabled_=(ack[12]&0x40)!=0;}}
...
#endif
 uint8_t h[12]{};h[0]=CMD_BLIT_FIELD_VSYNC;std::memcpy(h+1,&frame,4);h[5]=field;uint16_t vsync=0;std::memcpy(h+6,&vsync,2);if(csize)std::memcpy(h+8,&csize,4);size_t hs=csize?12:8;if(!sendPacket(h,hs,e)||!sendChunks(payload,bytes,e))return false;drainStatus();return true;}
...
void GroovyTransport::close()noexcept{if(fd_>=0){uint8_t c=CMD_CLOSE;std::string ignored;sendPacket(&c,1,ignored);::close(fd_);fd_=-1;}misterAudioEnabled_=false;frameBytes_=0;compressed_.clear();}
}
