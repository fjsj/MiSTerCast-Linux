...
void AudioPacer::reset(uint32_t sampleRate) noexcept {
  ...
  fractionalFrameNs_ = pendingFrames_ = 0;
}
...
size_t AudioPacer::valuesDue(uint64_t elapsedNs, size_t maxValues) noexcept {
  ...
  const uint64_t frameNumerator =
      fractionalFrameNs_ + remainderNs * sampleRate_;
...
size_t AudioPacer::sourceValuesFor(size_t outputValues, size_t bufferedValues,
                                   size_t targetValues,
                                   size_t hysteresisValues) const noexcept {
  if (!outputValues) return 0;
  if (bufferedValues > targetValues + hysteresisValues) return outputValues + 2;
  if (bufferedValues + hysteresisValues < targetValues && outputValues >= 4)
    return outputValues - 2;
  return outputValues;
}

void AudioPacer::conformStereo(const int16_t* source, size_t sourceValues,
  ...
  if (sourceValues == outputValues) {
    std::memcpy(output.data(), source, outputValues * sizeof(int16_t));
    return;
  }
  ...
  if (sourceValues == outputValues + 2) {
    ...
    std::memcpy(output.data() + pivot * 2, source + (pivot + 1) * 2,
                (outputFrames - pivot) * 2 * sizeof(int16_t));
    return;
  }
  if (sourceValues + 2 == outputValues) {
    std::memcpy(output.data(), source, (pivot + 1) * 2 * sizeof(int16_t));
    std::memcpy(output.data() + (pivot + 1) * 2, source + pivot * 2,
                2 * sizeof(int16_t));
    std::memcpy(output.data() + (pivot + 2) * 2, source + (pivot + 1) * 2,
                (sourceFrames - pivot - 1) * 2 * sizeof(int16_t));
    return;
  }
