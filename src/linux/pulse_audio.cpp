...
class PulseConnection {
 public:
  ...
  bool connect(std::string& error) {
    ...
    if (!loop_) {
      error = "cannot create PulseAudio main loop";
      return false;
    }
    ...
    if (pa_context_connect(context_, nullptr, PA_CONTEXT_NOFLAGS, nullptr) <
        0) {
      error = pa_strerror(pa_context_errno(context_));
      close();
      return false;
    }
    for (;;) {
...
bool serverAndSinks(PulseConnection& connection, ServerQuery& server,
  ...
  if (!connection.wait(pa_context_get_server_info(connection.context(),
                                                  serverInfo, &server)))
    return false;
  SinkQuery query{wanted.empty() ? server.defaultSink : wanted,
                  {},
                  sinks,
                  server.defaultSink};
...
void sinkInputInfo(pa_context*, const pa_sink_input_info* info, int end,
                   void* data) {
  if (!end && info)
    static_cast<std::vector<SinkInput>*>(data)->push_back(
...
class SilentRouting {
 public:
  bool start(std::string& monitor, std::string& error) {
    ...
    ServerQuery server;
    if (!connection_.wait(pa_context_get_server_info(connection_.context(),
                                                     serverInfo, &server)) ||
        server.defaultSink.empty()) {
      error = "PulseAudio has no default output sink";
      return false;
    }
    ...
    const std::string arguments =
        ...
        "sink_properties=device.description=MiSTerCast_Silent_Output";
    connection_.wait(pa_context_load_module(
        connection_.context(), "module-null-sink", arguments.c_str(),
        moduleCallback, &moduleIndex_));
    if (moduleIndex_ == PA_INVALID_INDEX) {
      error = "cannot create the MiSTerCast silent output sink";
      return false;
    }
    connection_.wait(pa_context_get_sink_info_by_name(
        connection_.context(), sinkName_.c_str(), sinkIndexInfo, &sinkIndex_));

    bool changedDefault = false;
    connection_.wait(
        pa_context_set_default_sink(connection_.context(), sinkName_.c_str(),
                                    successCallback, &changedDefault));
    if (!changedDefault) {
      error = "cannot select the MiSTerCast silent output sink";
      return false;
    }

    std::vector<SinkInput> inputs;
    if (connection_.wait(pa_context_get_sink_input_info_list(
            connection_.context(), sinkInputInfo, &inputs))) {
      for (const auto& input : inputs) {
        bool moved = false;
        connection_.wait(pa_context_move_sink_input_by_name(
            connection_.context(), input.index, sinkName_.c_str(),
            successCallback, &moved));
  ...
  void stop() noexcept {
    ...
    if (!originalDefault_.empty()) {
      bool restored = false;
      connection_.wait(pa_context_set_default_sink(connection_.context(),
                                                   originalDefault_.c_str(),
                                                   successCallback, &restored));
      std::vector<SinkInput> inputs;
      connection_.wait(pa_context_get_sink_input_info_list(
          connection_.context(), sinkInputInfo, &inputs));
      for (const auto& input : inputs) {
        if (input.sink != sinkIndex_) continue;
        bool moved = false;
        const auto previous =
            std::find_if(movedInputs_.begin(), movedInputs_.end(),
                         [&](const SinkInput& candidate) {
                           return candidate.index == input.index;
                         });
        if (previous != movedInputs_.end())
          connection_.wait(pa_context_move_sink_input_by_index(
              connection_.context(), input.index, previous->sink,
              successCallback, &moved));
        else
          connection_.wait(pa_context_move_sink_input_by_name(
              connection_.context(), input.index, originalDefault_.c_str(),
              successCallback, &moved));
      }
    }
    if (moduleIndex_ != PA_INVALID_INDEX) {
      bool unloaded = false;
      connection_.wait(pa_context_unload_module(
          connection_.context(), moduleIndex_, successCallback, &unloaded));
  ...
  bool start(const std::string& sink, ErrorCallback callback) override {
    ...
    if (sink == SilentAudioSink) {
      ...
      if (!silentRouting_->start(source, routeError)) {
        ...
        if (error_)
          error_({"audio", routeError,
                  "Ensure PulseAudio/pipewire-pulse permits loading "
                  "module-null-sink."});
        return false;
      }
    } else {
      PulseConnection connection;
      ServerQuery server;
      if (!connection.connect(routeError) ||
          !serverAndSinks(connection, server, nullptr, &source, sink) ||
          source.empty()) {
        if (error_)
          error_({"audio",
                  routeError.empty()
                      ? "selected output sink has no monitor source"
                      : routeError,
    ...
    if (!stream_) {
      ...
      if (error_)
        error_(
            {"audio", pa_strerror(pulseError),
             "Check pipewire-pulse/PulseAudio and the selected output sink."});
      return false;
    }
    running_ = true;
    return true;
    ...
    if (error_)
      error_({"audio",
              "PulseAudio development support was unavailable at build time",
...
#ifdef MISTERCAST_HAVE_PULSE
    if (timeout <= std::chrono::milliseconds::zero() || !running_ || !stream_)
      return false;
    const auto readMs = std::min<int64_t>(timeout.count(), 10);
    const auto frames =
        std::max<uint64_t>(1, uint64_t(rate_) * uint64_t(readMs) / 1000);
    block.sampleRate = rate_;
    block.samples.resize(size_t(frames) * 2);
    int pulseError = 0;
    if (pa_simple_read(stream_, block.samples.data(),
                       block.samples.size() * sizeof(int16_t),
                       &pulseError) < 0) {
      if (error_)
        error_({"audio", pa_strerror(pulseError),
                "Check pipewire-pulse/PulseAudio and restart streaming."});
      return false;
    }
